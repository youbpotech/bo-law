...
describe('cliente da API BotNiss', () => {
  it('adapta os campos operacionais documentados para o contrato do backoffice', () => {
    expect(
      mapNissProcess({
        ...
        status_operacional: 3,
        status_operacional_nome: 'CONCLUIDO',
  ...
  it('baixa o ficheiro pela rota documentada da API BotNiss', async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(
      new Response('conteúdo', {
        status: 200,
        headers: {
          'Content-Disposition': 'attachment; filename="713993_IDENTIFICACAO_FRENTE.pdf"',
    ...
    await expect(downloadNissDocument('4', '713993 IDENTIFICAÇÃO_FRENTE')).resolves.toEqual({
      data: Buffer.from('conteúdo'),
      fileName: '713993_IDENTIFICACAO_FRENTE.pdf',
      mimeType: 'application/pdf',
    })

    expect(fetchMock).toHaveBeenCalledWith(
      'http://botniss.test/api/v1/processos/4/documentos/713993%20IDENTIFICA%C3%87%C3%83O_FRENTE',
  ...
  it('interpreta o nome UTF-8 entregue no cabeçalho de download', () => {
    expect(
      fileNameFromContentDisposition(
        "attachment; filename*=UTF-8''comprovativo%20de%20atribui%C3%A7%C3%A3o.pdf",
      ),
    ).toBe('comprovativo de atribuição.pdf')
  })
})
