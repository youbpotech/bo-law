...
import type { NissDocument, NissDocumentDownload } from '@/composables/useApi'
...
interface Props {
  ...
  isLoading: boolean
  error: string
  baixarDocumento: (args: { processId: string; fileName: string }) => Promise<NissDocumentDownload>
...
async function downloadAll(): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < props.documents.length; i++) {
      ...
      try {
        const download = await props.baixarDocumento({
          processId: props.processId,
          fileName: doc.fileName,
        })
        const sanitizedName = download.fileName
          .replace(/^.*[\\/]/, '')
          .replace(/[^a-zA-Z0-9._ -]/g, '_')
        const name = `${String(i + 1).padStart(2, '0')}-${sanitizedName}`
        zip.file(name, download.blob)
      } catch {
    ...
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `pedido-niss-${props.requestNumber}-completo.zip`
    anchor.click()
    URL.revokeObjectURL(url)
  ...
  >
    <div class="space-y-4">
      <div class="flex items-start justify-between gap-4">
        ...
        <Button :disabled="isDownloadingAll || isLoading || !documents.length" @click="downloadAll">
      ...
      <p
        v-if="isDownloadingAll && downloadProgress"
        class="rounded-md border border-blue-600/20 bg-blue-600/5 p-3 text-sm text-blue-700 dark:text-blue-400"
      >
        {{ downloadProgress }}
      </p>
      ...
      <p v-if="isLoading" class="py-8 text-center text-muted-foreground">
        Carregando documentos...
      </p>
      <p v-else-if="error" class="rounded-md bg-destructive/5 p-3 text-sm text-destructive">
        {{ error }}
      </p>
      <p v-else-if="!documents.length" class="py-8 text-center text-muted-foreground">
